...
Note: This project is built with Vite; the service worker is generated by [vite-plugin-pwa](https://vite-pwa-org.netlify.app/) (Workbox).

 - Clone or download the repo
 - `npm install`
 - `npm run dev` to run the application with the Vite dev server, or `npm run build` to kick off a fresh build and update the output directory (`dist/angular-hnpwa/`)

Note: Any Service Worker changes will not be reflected when you run the application locally in development. To test service worker changes:
 - `npm run build`
 - `npm run preview` to serve the built application along with the generated service worker
