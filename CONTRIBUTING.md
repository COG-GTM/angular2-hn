...
6. Work your magic
7. Run `npm run test`, `npm run lint` and `npm run build` to make sure nothing is broken
8. To test service worker / PWA changes: `npm run build` then `npm run preview` to serve the production build (the service worker is only active in production builds)
