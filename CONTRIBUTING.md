...
4. Run `npm install`
5. Run `npm start` (or `npm run dev`) and open `http://localhost:5173` in a browser
6. Work your magic
7. Run `npm run lint`
8. Run `npm run build` to create a production build and make sure nothing is broken
9. To test service worker changes, run `npm run build && npm run preview` and open `http://localhost:4173`
