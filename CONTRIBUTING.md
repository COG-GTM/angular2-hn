...
4. `npm install`
5. Run `npm run dev` and open the printed `localhost` URL in a browser
6. Work your magic
7. Run `npm run lint`, `npm run typecheck` and `npm run build` to make sure nothing is broken
8. To test service worker changes: `npm run build && npm run preview`, since the service worker is only generated for production builds
