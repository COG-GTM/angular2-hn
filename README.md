...
This app uses [vite-plugin-pwa](https://vite-pwa-org.netlify.app/) (powered by [Workbox](https://developer.chrome.com/docs/workbox)) to generate a service worker as part of the build step to load quickly and work offline.
...
The app is built with [Vite](https://vitejs.dev/), React 18, TypeScript and React Router v6.

 - Clone or download the repo
 - `npm install`
 - `npm run dev` to run the application with the Vite dev server
 - `npm run build` to typecheck and produce a fresh production build in the output directory (`dist/`)
 - `npm test` to run the [Vitest](https://vitest.dev/) unit tests

Note: the service worker is only generated for production builds, so Service Worker changes are not reflected in `npm run dev`. To test them:
 - `npm run build`
 - `npm run preview` to serve `dist/` (service worker included) locally
