...
4. Install dependencies: `cd react && npm ci`
5. Run `npm run dev` and open `http://localhost:5173` in a browser
6. Work your magic
7. Run `npm run build` and make sure nothing is broken
8. Run `npm test` and `npm run e2e`
9. Run `npm run lint` and `npm run format:check`
10. To test service worker changes, run `npm run build` and serve the `react/dist` directory
