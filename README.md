...
This app uses [`vite-plugin-pwa`](https://vite-pwa-org.netlify.app/) (backed by [Workbox](https://workboxjs.org/)) to generate a service worker as part of the build step to load quickly and work offline.
...
This project uses [Vite](https://vitejs.dev/) with React and TypeScript.
 ...
 - `npm run build` to type-check and produce an optimized production build in the output directory (`dist/`)
 - `npm run preview` to locally serve the production build
 - `npm run lint` to run ESLint

Note: The service worker is only generated for production builds. To test the PWA / offline support locally, run `npm run build` followed by `npm run preview`.
