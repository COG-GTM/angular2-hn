...
4. `npm install`
5. `npm start` and open `localhost:4200` in a browser
6. Work your magic
7. Run `npm run lint` and `npm run build` to make sure nothing is broken
8. To test service worker changes (the service worker is only registered in production builds):
  * `npm run build` to produce a fresh `dist/`
  * `npm run preview` to serve it, then check `Application -> Service Workers` in devtools
9. Add yourself to the contributor's list in the README!
