...
4. `cd react-app && npm install`
5. Run `npm run dev` and open the printed URL in a browser
6. Work your magic
7. Run `npm test` and `npm run build` to make sure nothing is broken
8. To test service worker changes (the service worker is only generated for production builds):
  * `npm run build` to kick off a fresh build into `react-app/dist`
