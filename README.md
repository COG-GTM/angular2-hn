...
This project uses [Vite](https://vitejs.dev/) with React + TypeScript. The service worker is generated by [`vite-plugin-pwa`](https://vite-pwa-org.netlify.app/) (Workbox).

 - Clone or download the repo (requires Node 18+)
 - `npm install`
 - `npm run dev` to start the Vite dev server (http://localhost:3000)
 - `npm run build` to type-check and produce a production build in `dist/react`
 - `npm run preview` to serve the production build locally
 - `npm run lint` / `npm run typecheck` for linting and type checking

Note: The service worker is only enabled in production builds. To test offline behavior, run `npm run build` followed by `npm run preview`.
