...
4. Run `npm install`
5. Run `npm run dev` (or `npm start`) and open `localhost:5173` in a browser
6. Work your magic
7. Run `npm run lint` and `npm test` to make sure nothing is broken
8. Run `npm run build` to kick off a production build, and `npm run preview` to serve it locally
